...
string calculateSimpleHash(const string& filepath) {
    ...
    if (result.length() < HASH_SIZE) {
        result.append(HASH_SIZE - result.length(), '0');
    } else if (result.length() > HASH_SIZE) {
        result = result.substr(0, HASH_SIZE);
    }
        ...
        for (const auto& entry : fs::recursive_directory_iterator(config.directory)) {
            if (entry.is_regular_file()) {
                ...
                if (matchesExtension(filepath, config.extensions)) {
                    ...
                    string hash = calculateSimpleHash(filepath);
